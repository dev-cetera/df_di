...
base mixin SupportsConstructorsMixinT
  ...
  @protected
  @pragma('vm:prefer-inline')
  Option<Lazy<T>> getLazySyncOrNoneT<T extends Object>(
    Type type, {
    Entity groupEntity = const DefaultEntity(),
    bool traverse = true,
  }) {
    return getSyncOrNoneT<Lazy<T>>(
      type,
      groupEntity: groupEntity,
      traverse: traverse,
    );
  ...
  @pragma('vm:prefer-inline')
  Option<T> getLazySingletonSyncOrNoneT<T extends Object>(
    Type type, {
    Entity groupEntity = const DefaultEntity(),
    bool traverse = true,
  }) {
    return getLazySingletonSyncOrNoneK<T>(
      TypeEntity(type),
      groupEntity: groupEntity,
      traverse: traverse,
    );
  ...
  /// instance as an [Option].
  @pragma('vm:prefer-inline')
  Option<T> getLazyFactorySyncOrNoneT<T extends Object>(
    Type type, {
    Entity groupEntity = const DefaultEntity(),
    bool traverse = true,
  }) {
    return getLazyFactorySyncOrNoneK<T>(
      TypeEntity(type),
      groupEntity: groupEntity,
      traverse: traverse,
    );
